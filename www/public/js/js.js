$(document).ready(function() {         
    ...
    $promoform.submit(function(event) {
        ...
        if(email.length == 0) {
            $tooltip.text(messages['empty_email']).addClass('error').appendTo($promoform);
        }
        else {
            $button.attr('disabled', 'disabled');

            $.post('promosubscribe.php', {
                'email': email,
                'code': code,
                'wallet': wallet,
                'ajax': 1
            ...
            function(data) {
                if(data == null || typeof(data.status) == 'undefined' || (data.status == 'error' && typeof(data.error) == 'undefined')) {
                    $tooltip.text(messages['default']).addClass('error').appendTo($promoform);
                }
                else if(data.status == 'success') {
                    $tooltip.text(messages['success']).addClass('success').appendTo($promoform);
                }
                else {
                    var error_text = messages['default_error'];
                    ...
                    $tooltip.text(error_text).addClass('error').appendTo($promoform);
                }
